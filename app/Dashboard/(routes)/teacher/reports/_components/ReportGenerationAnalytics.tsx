...
const ReportGenerationAnalytics = ({
  ...
  const generateChart = () => {
    const chartInstance = new Chart(chartRef.current as HTMLCanvasElement, {
      ...
      data: {
        labels: ["5G Communication System Using Matlab", "The Basics of Rocket Science"],
        datasets: [
          {
            label: "total",
  ...
  const generatePDF = () => {
    ...
    doc.roundedRect(startX, startY - 10, borderWidth, borderHeight, borderRadius, borderRadius);
    ...
    doc.setFontSize(13);
    doc.text(`For the month starting from: ${selectedMonth}`, startX + 63.5, startY + 28);
  
    // Add data cards for revenue and sales
    doc.setFontSize(15);
    doc.text(`Total Revenue : $${totalRevenue.toFixed(2)}`, startX + 10, startY + 45);
    doc.text(`Total Sales      : ${totalSales}`, startX + 10, startY + 52);
    ...
    if (chartImage) {
      doc.addImage(chartImage, "PNG", startX + 10, startY + 90, 170, 75); // Adjust size and position as needed
    }
  
    // Add legend
    doc.setFontSize(12);
    doc.setTextColor(0, 200, 159); // Green color
    doc.text("Revenue > $60: Green", 10, startY + 180);
    doc.setTextColor(255, 191, 40); // Yellow color
    doc.text("Revenue > $20: Yellow", 70, startY + 180);
    doc.setTextColor(255, 128, 66); // Orange color
    doc.text("Revenue <= $20: Orange", 140, startY + 180);
