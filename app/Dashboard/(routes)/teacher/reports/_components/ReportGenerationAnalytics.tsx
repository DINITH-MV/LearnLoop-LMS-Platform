...
const ReportGeneration = ({ feedbacks }: ReportGenerationProps) => {
  ...
  const generateChart = () => {
    ...
    const chartInstance = new Chart(chartRef.current as HTMLCanvasElement, {
      ...
      data: {
        labels: ["Replied", "Non-Replied"],
        datasets: [
          {
            label: "Feedback",
            data: [repliedFeedbacks.length, nonRepliedFeedbacks.length],
  ...
  const generatePDF = () => {
    ...
    const chartImage = chartRef.current?.toDataURL("image/png"); // Convert canvas to Base64 image

    // Add chart image to PDF
    if (chartImage) {
      doc.addImage(chartImage, "PNG", 10, startY + 80, 150, 75); // Adjust size and position as needed
    }
  ...
  return (
    <div className="mt-4 mb-6 flex gap-2">
      ...
      />
      <Button onClick={generatePDF} size="default">
        View PDF Report
      </Button>

      {/* Hidden canvas for rendering the chart */}
