...
import { font } from "./RobotoCondensed-normal";
...
const FeedbackReportGenerator = ({ feedbacks }: ReportGenerationProps) => {
  ...
  const generatePDF = () => {
    ...
    const startY = 20;
    const rowHeight = 8;
    const columnWidths = [70, 70, 40]; // Adjusted for narrower columns
    const borderWidth = 190; // Adjusted based on new table width
    ...
    doc.text("COURSE FEEDBACK REPORT", startX + 60, startY + 20); // Adjusted title position
    doc.setFontSize(15);
    doc.text(
    ...
    doc.setFontSize(15);
    doc.text(`Replies Count: ${repliedFeedbacks.length}`, startX + 5, startY + 50);
    doc.text(`Non-Replies Count: ${nonRepliedFeedbacks.length}`, startX + 5, startY + 60);
    ...
    const generateTable = (feedbackData: Feedback[], title: string, yPos: number) => {
      ...
      doc.rect(startX + 5, yPos, columnWidths.reduce((a, b) => a + b, 0), rowHeight, "F");

      // Align header text to left
      doc.text("Message", startX + 15, yPos + 6); 
      doc.text("Reply", startX + columnWidths[0] + 2, yPos + 6);
      doc.text("Created At", startX + columnWidths[0] + columnWidths[1] + 2, yPos + 6);
      ...
      feedbackData.forEach((feedback, index) => {
        ...
        doc.text(feedback.messages.substring(0, 25), startX + 7, yPos + 6);
        doc.text(feedback.reply.substring(0, 15), startX + columnWidths[0] + 2, yPos + 6);
        doc.text(format(parseISO(feedback.createdAt), "yyyy-MM-dd"), startX + columnWidths[0] + columnWidths[1] + 2, yPos + 6);
